...
import {
  ...
  createMeteredUsageSignaturePayload,
  quoteMeteredUsage,
...
describe('L402Agent', () => {
  ...
  it('lists offers with reputation filters', async () => {
    vi.spyOn(Date, 'now').mockReturnValue(1_780_320_000_000);
    ...
    const path = '/api/v1/offers?sort=reputation&min_reputation=75&hide_unrated=true&limit=10';
    const expectedSignature = createHmac('sha256', 'test-key')
      .update(`GET\n${path}\n1780320000000\n`)
      .digest('hex');
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        headers: expect.objectContaining({
          'X-L402-Key': 'test-key',
          'X-L402-Timestamp': '1780320000000',
          'X-L402-Signature': expectedSignature,
        }),
      })
    );
  });

  it('signs authenticated POST requests over method, path, timestamp, and body', async () => {
    vi.spyOn(Date, 'now').mockReturnValue(1_780_320_123_456);
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({ invoice: 'lnbc...', payment_hash: 'hash-1', amount_sats: 1234 }),
    } as Response);

    const agent = new L402Agent({ apiKey: 'test-key', apiUrl: 'https://api.example.com' });
    await agent.createDeposit(1234);

    const body = JSON.stringify({ amount_sats: 1234 });
    const expectedSignature = createHmac('sha256', 'test-key')
      .update(`POST\n/api/v1/wallet/deposit\n1780320123456\n${body}`)
      .digest('hex');
    const [, init] = fetchMock.mock.calls[0];

    expect(fetchMock.mock.calls[0][0]).toBe('https://api.example.com/api/v1/wallet/deposit');
    expect(init).toMatchObject({
      method: 'POST',
      body,
      headers: expect.objectContaining({
        'X-L402-Key': 'test-key',
        'X-L402-Timestamp': '1780320123456',
        'X-L402-Signature': expectedSignature,
      }),
    });
  });
  ...
  it('browses marketplace offers without tenant auth', async () => {
    ...
    expect((init?.headers as Record<string, string>)['X-L402-Timestamp']).toBeUndefined();
    expect((init?.headers as Record<string, string>)['X-L402-Signature']).toBeUndefined();
  });

  it('createOffer only sends allowlisted offer fields', async () => {
    vi.spyOn(Date, 'now').mockReturnValue(1_780_320_222_333);
    const fetchMock = vi.spyOn(globalThis, 'fetch').mockResolvedValue({
      ok: true,
      json: async () => ({
        id: 'offer-1',
        ...
        terms: { sla_minutes: 45, dispute_window_minutes: 120 },
        price_sats: 500,
    ...
    const params = JSON.parse(`{
      ...
      "dispute_window_minutes": 120,
      "active": 0,
      "seller_tenant_id": "attacker",
      "admin": true,
      "__proto__": { "polluted": true }
    }`);

    const agent = new L402Agent({ apiKey: 'test-key', apiUrl: 'https://api.example.com' });
    await agent.createOffer(params);
    ...
    expect(body).not.toHaveProperty('seller_tenant_id');
    expect(body).not.toHaveProperty('admin');
    expect(body).not.toHaveProperty('__proto__.polluted');
  });
  ...
  it('quotes metered usage with BigInt intermediate pricing math', () => {
    const card = createTokenServiceCard({
      ...tokenServiceCardOptions(),
      pricing: {
        ...tokenServiceCardOptions().pricing,
        input_sats: 102,
        output_sats: 0,
        request_minimum_sats: 0,
      },
    });
    const contract = createMeteredEscrowContract({
      issuedAt: '2026-05-30T22:10:00.000Z',
    ...
    const quote = quoteMeteredUsage(contract, {
      requestId: 'req-bigint-pricing',
      modelId: 'seller/coding-large',
      inputTokens: 9_007_199_254_730_991,
      outputTokens: 0,
    });

    expect(quote.input_sats).toBe(918_734_323_982_562);
    expect(quote.total_sats).toBe(918_734_323_982_562);
  });

  it('applies seller-signed metered usage only with a valid signature', () => {
    const { publicKey, privateKey } = generateKeyPairSync('ed25519');
    const publicKeyPem = publicKey.export({ format: 'pem', type: 'spki' }).toString();
    const card = createTokenServiceCard({
      ...tokenServiceCardOptions(),
      metering: {
        ...tokenServiceCardOptions().metering,
        method: 'seller_signed_usage',
      },
    });
    const contract = createMeteredEscrowContract({
    ...
    const payload = createMeteredUsageSignaturePayload(contract, usage, quote);
    const sellerSignature = sign(null, Buffer.from(payload), privateKey).toString('base64');

    const result = applyMeteredUsage(contract, {
      ...usage,
      sellerSignature,
      sellerPublicKeyPem: publicKeyPem,
    });

    expect(result.applied).toBe(true);
    expect(result.codes).toEqual(['applied']);
    expect(result.event?.seller_signature).toBe(sellerSignature);
    expect(result.event?.seller_public_key_pem).toBe(publicKeyPem);
    expect(verifyMeteredEscrowContract(result.contract).valid).toBe(true);
  });

  it('rejects missing and invalid seller-signed metered usage signatures', () => {
    const { publicKey, privateKey } = generateKeyPairSync('ed25519');
    const publicKeyPem = publicKey.export({ format: 'pem', type: 'spki' }).toString();
    const card = createTokenServiceCard({
      ...tokenServiceCardOptions(),
    ...
    const missingSignature = applyMeteredUsage(contract, { ...usage, sellerPublicKeyPem: publicKeyPem });
    expect(missingSignature.applied).toBe(false);
    expect(missingSignature.codes).toContain('missing_signature');

    const missingPublicKey = applyMeteredUsage(contract, {
      ...usage,
      sellerSignature: sign(null, Buffer.from('wrong payload'), privateKey).toString('base64'),
    });
    expect(missingPublicKey.applied).toBe(false);
    expect(missingPublicKey.codes).toContain('missing_public_key');

    const invalidSignature = applyMeteredUsage(contract, {
      ...usage,
      sellerSignature: sign(null, Buffer.from('wrong payload'), privateKey).toString('base64'),
      sellerPublicKeyPem: publicKeyPem,
    });
    expect(invalidSignature.applied).toBe(false);
    expect(invalidSignature.codes).toContain('invalid_signature');
  });

  it('rejects unsafe integer metered usage instead of rounding it', () => {
    const card = createTokenServiceCard(tokenServiceCardOptions());
    const contract = createMeteredEscrowContract({
      issuedAt: '2026-05-30T22:10:00.000Z',
      tokenServiceCard: card,
    ...
    const unsafeUsage = {
      requestId: 'req-unsafe-integer',
      modelId: 'seller/coding-large',
      inputTokens: Number.MAX_SAFE_INTEGER + 1,
      outputTokens: 0,
    };

    expect(() => quoteMeteredUsage(contract, unsafeUsage)).toThrow(RangeError);
    const result = applyMeteredUsage(contract, unsafeUsage);
    expect(result.applied).toBe(false);
    expect(result.codes).toContain('invalid_usage');
  ...
  it('flags unsafe integer pricing in metered escrow contracts', () => {
    const card = createTokenServiceCard(tokenServiceCardOptions());
    const contract = createMeteredEscrowContract({
      issuedAt: '2026-05-30T22:10:00.000Z',
      tokenServiceCard: card,
    ...
    contract.pricing.input_sats = Number.MAX_SAFE_INTEGER + 1;
